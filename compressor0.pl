...
increment(X, Times, [Y|Result], Rest):-	%%%%%%%%% to be written %%%%%%%%%%%
	plusOne(X,Y),
	increment(Y,NTimes,Result,Rest),
	plusOne(NTimes,Times).
...
testCompress2 :-
    ...
    complexity(L,CL,_),
	complexity(CL,CCL,_),
        write('2-times compressed version:\t'), write(CCL),nl,
    compress(CL1,CCL),  % Funny: 'compress' is used to decompress
	compress(L1,CL1),  % Funny again: 'compress' is used to decompress
    !,  % necessary, since reversibility is not perfect
        write('2-times recovered list:  \t'), write(L1),nl.
